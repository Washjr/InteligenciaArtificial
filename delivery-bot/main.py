...
def rodar_simulacao(seed, player_class):
    world = World(seed=seed, render=False, player_class=player_class)
    maze = Maze(world, render=False)
    resultado = maze.game_loop()
    resultado["seed"] = seed
    return resultado

def simulacao_monte_carlo(n_simulacoes=100, player_class=None):
    resultados = []
    for _ in range(n_simulacoes):
        seed = random.randint(0, 100000)
        resultado = rodar_simulacao(seed, player_class)
